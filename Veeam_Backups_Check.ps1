...
switch ($state) {
    ...
    idle {
        ...
        $rto = (Get-Date).AddDays(-$period)
        if ((get-date $lastOkSession.EndTime) -ge (get-date $rto) -and ($lastOkSession.State -ne "failed")) {
            Write-Host "OK! $name waiting for new restore points."
            exit 0
        } 
        elseif ((get-date $lastOkSession.EndTime) -lt (get-date $rto) -and ($lastOkSession.State -ne "failed")) {
    ...
    stopped {
        #recovery time objective 
        $rto = (Get-Date).AddDays(-$period)
        if ((get-date $lastOkSession.EndTime) -gt (get-date $rto) -and ($lastOkSession.State -ne "failed")) {
            Write-Host "OK! $name waiting for new restore points."
            exit 0
        } 
        elseif ((get-date $lastOkSession.EndTime) -le (get-date $rto) -and ($lastOkSession.State -ne "failed")) {
